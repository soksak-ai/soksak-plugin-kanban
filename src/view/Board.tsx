  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 4, padding: "12px 22px 0", flexWrap: "wrap" }}>
        {focusId && (
          <button onClick={goUp} title="상위로" style={{ height: 26, padding: "0 9px", border: "1px solid var(--border)", borderRadius: 7, background: "var(--bg)", color: "var(--text-2)", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", marginRight: 4 }}>↑ 상위로</button>
        )}
          ...
          return (
            <div key={col.id} style={{ width: 278, flex: "none", background: "var(--surface-2)", borderRadius: "var(--r-col)", border: col.bottleneck ? "1px solid rgba(245,158,11,.4)" : "1px solid var(--border)", display: "flex", flexDirection: "column", maxHeight: "100%" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px 9px" }}>
                ...
                <span style={{ fontSize: 11, color: "var(--text-3)" }}>{col.kr}</span>
              ...
              >
                ...
                {col.cards.map((card) => (
                  <Card key={card.id} card={card} dragging={dragId === card.id} onDragStart={() => setDragId(card.id)} onDragEnd={() => { setDragId(null); setOverStatus(null); }} onSelect={() => (hasChildren(nodes, card.id) ? setFocusId(card.id) : onOpen(card.id))} onDrill={() => setFocusId(card.id)} />
                ))}
                <button onClick={() => onCreate(col.id)} style={{ width: "100%", padding: 8, border: "1px dashed var(--border-2)", borderRadius: "var(--r-card)", background: "transparent", color: "var(--text-3)", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>+ 카드 추가</button>
              </div>
            </div>
          );
        })}
...
function Card({ card, dragging, onDragStart, onDragEnd, onSelect, onDrill }: { card: CardVM; dragging: boolean; onDragStart: () => void; onDragEnd: () => void; onSelect: () => void; onDrill: () => void }) {
  return (
    <div
      draggable
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
        ...
        {card.stale && <span style={{ marginLeft: "auto", fontSize: 9.5, fontWeight: 700, color: "#f59e0b", background: "rgba(245,158,11,.14)", padding: "1px 6px", borderRadius: 6 }}>{card.staleDays}일 정체</span>}
      </div>
      <div style={{ fontSize: 13, fontWeight: 500, lineHeight: 1.4 }}>{card.title || "(제목 없음)"}</div>
