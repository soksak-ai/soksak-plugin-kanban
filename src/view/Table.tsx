...
import { PRIORITY, resolveLabel } from "@/refs";
import { t } from "@/view/i18n";

type ColDef = [SortKey, { en: string; ko: string }, number];
const COLS: ColDef[] = [
...
export default function Table({ nodes, onOpen, lang }: { nodes: Node[]; onOpen: (id: string) => void; lang: string }) {
  ...
  return (
    <div style={{ padding: "20px 22px", height: "100%", overflow: "auto" }}>
      <h3 style={{ margin: "0 0 18px", fontSize: 14, fontWeight: 600 }}>{t("tableTitle", lang)}</h3>
      <div style={{ border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden", background: "var(--surface)" }}>
        <div style={{ display: "flex", alignItems: "center", padding: "0 16px", height: 40, borderBottom: "1px solid var(--border)", background: "var(--surface-2)" }}>
          {COLS.map(([k, lbl, w]) => (
            <div key={k} onClick={() => onSort(k)} style={{ width: k === "title" ? "auto" : w, flex: k === "title" ? 1 : "none", display: "flex", alignItems: "center", gap: 4, cursor: "pointer", fontSize: 11, fontWeight: 600, color: sortKey === k ? "var(--text)" : "var(--text-3)", paddingLeft: k === "key" ? 48 : 0 }}>
              <span>{(lbl as Record<string, string>)[lang] ?? lbl.en}</span>
        ...
        {rows.map((r) => (
          <div key={r.id} onClick={() => onOpen(r.id)} style={{ display: "flex", alignItems: "center", padding: "0 16px", height: 44, borderBottom: "1px solid var(--grid)", cursor: "pointer", fontSize: 13 }}>
            ...
            <div style={{ width: 104, flex: "none" }}><span style={statusChip(r.status)}>{resolveLabel(sMeta(r.status).label, lang)}</span></div>
            ...
            <div style={{ width: 78, flex: "none", display: "flex", alignItems: "center", gap: 6 }}><span style={prDot(r.priority)} /><span style={{ fontSize: 12, color: "var(--text-2)" }}>{resolveLabel(PRIORITY[r.priority].label, lang)}</span></div>
